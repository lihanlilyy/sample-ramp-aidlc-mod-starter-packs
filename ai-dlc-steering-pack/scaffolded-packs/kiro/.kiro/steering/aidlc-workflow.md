...
## MANDATORY: Diagram Standard (Zoomable & Readable)

Applies to **every diagram generated in any phase** — Phase 0 analysis,
`requirements.md`, `design.md`, and the `tasks.md` parallelism diagram. A diagram
that renders too small to read is a defect: a large Mermaid graph embedded inline
collapses to an unreadable thumbnail in most viewers. Make every diagram openable
and zoomable — follow ALL of these:

1. **Also save each diagram as its own standalone file** in a `diagrams/`
   subfolder next to the spec documents (e.g. `diagrams/architecture.mmd` holding
   the Mermaid source), in addition to embedding it in the document. A standalone
   file opens at full size and can be zoomed and panned independently of the doc.
2. **Put a zoom link directly beneath every embedded diagram**, for example:
   > 🔍 **[Open full-size / zoomable diagram](diagrams/architecture.mmd)** — the inline view above may render too small to read.

   When practical, also offer a **[View & edit in Mermaid Live](https://mermaid.live/)** link so the reader can paste the source and zoom/pan interactively.
3. **Keep each diagram focused — one concern per diagram.** If a diagram exceeds
   ~15 nodes or two levels of nesting, SPLIT it (e.g. a high-level context diagram
   plus per-area detail diagrams) rather than emitting one dense, unreadable graph.
4. **Favor legible layouts:** prefer `graph TD` (top-down), keep node labels
   short, and group related nodes with `subgraph` blocks.
5. **Readable beats complete-in-one-picture** — never rely on a single giant
   diagram to convey the whole system.
...
**Diagrams:** if you include any diagram here (e.g. a scope/context or
user-journey diagram), it MUST follow the **Diagram Standard (Zoomable &
Readable)** above — save it as a standalone file in `diagrams/`, add a zoom link
beneath the embedded copy, and split anything too dense to read.
...
Every diagram above MUST follow the **Diagram Standard (Zoomable & Readable)** —
save each as a standalone file in `diagrams/`, add a zoom link directly beneath
the embedded copy, and split any diagram too dense to read at normal zoom into
smaller focused diagrams. Do not emit a single oversized diagram that renders as
...
### Parallelism Flow Diagram

<!-- Mermaid diagram generated HERE, on top of the task list — see "MANDATORY: Parallelism Flow Diagram" below.
     Render each wave as a subgraph (groups inside a wave run concurrently); draw arrows only for cross-wave dependencies.
     Follow the Diagram Standard: also save as diagrams/tasks-parallelism.mmd and add a zoom link beneath it. -->
...
**MANDATORY: Parallelism Flow Diagram**

`tasks.md` MUST include a Mermaid flow diagram **on top of the task list**
(immediately after the Execution Plan table) that visualizes the waves and the
parallel groups within each wave. This makes the parallelism obvious at a glance:
which groups can run concurrently, and which must wait.

- Wrap each wave in a `subgraph` — groups inside the same subgraph have zero
  shared state and run concurrently on separate workers.
- Draw arrows ONLY for real cross-wave dependencies (a group that consumes
  another group's output).
- It MUST follow the **Diagram Standard (Zoomable & Readable)**: also save it as
  `diagrams/tasks-parallelism.mmd` and add a zoom link directly beneath it.

Template (customize group names/labels to the actual plan):

```mermaid
graph TD
    subgraph Wave1["Wave 1 — run all in parallel (no dependencies)"]
        A["Group A: &lt;feature&gt;"]
        B["Group B: &lt;feature&gt;"]
        C["Group C: &lt;feature&gt;"]
    end
    subgraph Wave2["Wave 2 — starts after Wave 1 completes"]
        D["Group D: &lt;feature&gt;"]
        E["Group E: &lt;feature&gt;"]
    end
    subgraph Wave3["Wave 3 — integration &amp; verification"]
        F["Group F: verify + integrate"]
    end
    A --> D
    B --> D
    B --> E
...
> 🔍 **[Open full-size / zoomable diagram](diagrams/tasks-parallelism.mmd)** — groups within a wave run concurrently on separate workers; arrows are cross-wave dependencies.
