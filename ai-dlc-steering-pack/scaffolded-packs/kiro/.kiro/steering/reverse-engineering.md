...
Existing tests are first-class signal for any modernization, migration, or
feature work — they document current behavior and form a safety net. Inventory
what already exists before planning changes, so downstream work can build on,
extend, or deliberately replace it rather than duplicating or breaking it. Skip
only if a scan confirms there are genuinely no tests (and say so).
...
### [Suite / package name]
- **Location**: [path]
- **Type**: [Unit / Integration / Contract / End-to-end / Load / Other]
- **Framework + version**: [e.g. JUnit 5, pytest 8, Jest 29, Go test, RSpec]
- **Language**: [language + binding]
- **Count**: [approx number of tests]
- **What it covers**: [features / modules / flows under test]
...
- **By layer**: [Unit / Integration / End-to-end — which exist, rough strength]
- **Notable gaps**: [Critical modules or flows with NO automated coverage]
...
- **Reporting**: [Report / dashboard / none]

## Test Data & Isolation
- **Provisioning**: [API seeding / DB fixtures / hardcoded / unclear]
- **Isolation**: [Per-test / shared fixtures / order-dependent]
- **Environment**: [Dedicated test env / shared staging / local only]

## Reliability Signals
- **Known flaky / quarantined / skipped tests**: [List with locations if visible —
  look for `.skip`, `.only`, `@Disabled`, `xit`, retry config]
- **Timing hacks**: [Presence of `sleep`, `Thread.sleep`, fixed waits]

## Migration / Survivability (if an upgrade or framework migration is planned)
- **Tests likely to survive**: [Behavior / contract tests decoupled from framework internals]
- **Tests likely to break**: [Framework-coupled tests a planned migration would invalidate]
...
- **Biggest opportunity**: [Where additional tests add the most value given what exists]
```
